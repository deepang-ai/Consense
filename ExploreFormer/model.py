...
class GraphTransformer(nn.Module):
    def __init__(self, 
                 in_size, num_class, d_model, 
                 num_heads=8, num_layers=4,
                 dim_feedforward=256, 
                 dropout=0.1,
                 batch_norm=True,
                 abs_pe=False, abs_pe_dim=0,
                 gnn_type="gat", se="khopgnn", 
                 use_edge_attr=False, num_edge_features=4,
                 in_embed=False, edge_embed=False, 
                 use_global_pool=True, global_pool='mean', 
                 act_func=nn.ReLU, trans_act_func='relu',
                 **kwargs):
        super().__init__()
        ...
        if global_pool == 'mean':
            ...
            self.pooling = gnn.GlobalAttention(nn.Linear(d_model, 1))
